  ...
  \begin{enumerate}[label=(\alph*)]
    ...
    \begin{enumerate}[label=(\roman*)]
      ...
      \LTR\begin{prooftree}
        ...
        \UIC{$B \rightarrow C$}
      \end{prooftree}\RTL
      فرض کنیم $v(A) = v(B \rightarrow C) = 0$ است، در نتیجه، طبق جدول درستی با $v(C) = 0$ باشد. براساس فرض استقرا مقدمهٔ حذف نشده‌ای مانند $B'$ در $C$ وجود دارد که $v(B') \leq v(C)$. واضح است که $v(B') \leq v(A)$، اما برای این که مطمئن باشیم $B'$ مقدمهٔ $A$ هم هست، باید نشان دهیم $B \neq B'$. اما می‌دانیم $v(B \rightarrow C) = 0$، پس طبق جدول درستی $v(B)$ نمی‌تواند $0$ باشد، در حالی که $v(B') \leq v(C) = 0$.

      \item فرض کنیم $v(A) = v(B \rightarrow C) = \frac{1}{2}$، که مستلزم آن است که $v(B) = 1$ و $v(C) = \frac{1}{2}$. باز هم با استدلال مشابه، بر اساس فرض استقرا، مقدمهٔ حذف نشده‌ای برای $C$ وجود دارد که در شرایط حکم صدق می‌کند و مقدمه‌ای برای $A$ نیز هست.
    \end{enumerate}

    \item بر اساس نتیجهٔ بخش (د)،‌ اگر بتوان قانون پرس را در دستگاهی که فقط شامل ادات شرطی است اثبات کرد، باید ارزش آن در هر ارزیاب سه-ارزشی برابر با $1$ باشد. اما طبق نتیجهٔ بخش (ج) می‌دانیم که در یکی از ارزیاب‌ها این مقدار برابر با $\frac{1}{2}$ است. پس قانون پرس در این دستگاه قابل اثبات نیست.
    \item کافی است که تابع ارزیابی را در نظر بگیریم که  
    $v(p) = 1/2$
  \end{enumerate}
\end{ans}
